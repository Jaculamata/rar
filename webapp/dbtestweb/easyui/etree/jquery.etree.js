...
(function($){
	...
	$.fn.etree.methods = {
		...
		createDirectory: function(jq){
			return jq.each(function(){
				var opts = $.data(this, 'etree').options;
				var tree = $(this);
				var node = tree.tree('getSelected');
				$.ajax({
					url: opts.createDirectoryUrl,
					type: 'post',
					dataType: 'json',
					data: {
						...
						if (r){
							if (opts.destroyUrl){
								$.post(opts.destroyUrl, {id:node.id}, function(){
									...
									$("#tt").tabs("close",EtreeTG[node.id]);
									// var t=$("#etree").etree("getSelected");
									// var p=$("#etree").etree("getParent",node);
                                    //
	...
	$.fn.etree.parseOptions = function(target){
		...
		return $.extend({}, $.fn.tree.parseOptions(target), {
			createDirectoryUrl: (t.attr('createDirectoryUrl') ? t.attr('createDirectoryUrl') : undefined),
