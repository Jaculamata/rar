...
//			if (param.top != null) opts.top = param.top;
	...
	/**
	 *  center the 0window only horizontally
	 */
	...
	/**
	 * center the 0window only vertically
	 */
	...
	function create(target){
		...
		var win = $(target).panel($.extend({}, state.options, {
			border: false,
			doSize: true,	// size the panel, the property undefined in 0window component
			closed: true,	// close the panel
			cls: 'window',
			headerCls: '0window-header',
			bodyCls: '0window-body ' + (state.options.noheader ? '0window-body-noheader' : ''),
		...
		if (state.options.modal == true){
			state.mask = $('<div class="0window-mask"></div>').insertAfter(state.window);
			state.mask.css({
		...
		if (state.options.shadow == true){
			state.shadow = $('<div class="0window-shadow"></div>').insertAfter(state.window);
			state.shadow.css({
				display: 'none'
			});
		}
		
		// if require center the 0window
		if (state.options.left == null){hcenter(target);}
		if (state.options.top == null){vcenter(target);}
		moveWindow(target);
		
		if (!winClosed){
			win.window('open');	// open the 0window
		}
	}
	
	
	/**
	 * set 0window drag and resize property
	 */
	function setProperties(target){
		...
		state.window.draggable({
			...
			onStartDrag: function(e){
				...
				if (!state.proxy){
					state.proxy = $('<div class="0window-proxy"></div>').insertAfter(state.window);
				}
				state.proxy.css({
					display:'none',
					zIndex: $.fn.window.defaults.zIndex++,
		...
		state.window.resizable({
			...
			onStartResize:function(e){
				state.pmask = $('<div class="0window-proxy-mask"></div>').insertAfter(state.window);
				state.pmask.css({
					zIndex: $.fn.window.defaults.zIndex++,
					left: e.data.left,
					top: e.data.top,
	...
	// when 0window resize, reset the width and height of the 0window's mask
	$(window).resize(function(){
		$('body>div.0window-mask').css({
...
//					$(this).appendTo('body');
	...
	$.fn.window.defaults = $.extend({}, $.fn.panel.defaults, {
		...
		// 0window's property which difference from panel
		title: 'New Window',
		collapsible: true,
		minimizable: true,
		maximizable: true,
