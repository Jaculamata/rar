...
public class MoreRunnable implements Runnable{
    ...
    public boolean AnalyseNode(Node m_XmlNode)
    {
        ...
        switch(m_XmlNode.getNodeName().toUpperCase())             //将m_XmlNode节点的标签统一改成大写
        {
            ...
            case "TEMPVAL":
                ...
                if(cRunInfo.containsKey("TEMPVAL"))
                    sTempStr = cRunInfo.get("TEMPVAL").toString();

                if(cRunInfo.containsKey("TEMPVAL"))
                    cRunInfo.remove("TEMPVAL");       //防止自身被替代
    ...
    public String ReplaceRunInfo(String sql)
    {
        ...
        if(cRunInfo.containsKey("TEMPVAL"))
        {
            tmp = cRunInfo.get("TEMPVAL");
            cRunInfo.remove("TEMPVAL");
            sql = sql.replace("@TEMPVAL",tmp.toString());          //提前替换
        }
        ...
        if(tmp != null)
        {
            cRunInfo.put("TEMPVAL",tmp.toString());
        }
        return sql;
    }
    ...
    public boolean  ExecuteSQL(String m_Sql)
    {
            ...
            try
            {
                ...
                if (!m_noShow)
                {
                    parentProcess.frame.AppendMess(m_Sql, true);
                    if(parentProcess.messRet != null)
                        parentProcess.messRet.callback(parentProcess.User_ID,parentProcess.stXmlRunInfo.sXmlFileName, m_Sql);
                }
                currConnectInfo.isOpenResult = false;
                time1 = new Date();                         //为执行语句计时
                currConnectInfo.State = currConnectInfo.cn.createStatement();
                if(currConnectInfo.State.execute(m_Sql))                              //有结果集返回
                {
    ...
    private void RunImportfun(Node m_XmlNode)
    {
        ...
        try {
            ...
            parentProcess.ShowError("第三方Jar调用失败，错误信息：" + e.getMessage());
        }
        if(ret != null)
        {
            ...
            parentProcess.pro_config.setVal(GetNodeText(retname), ret.toString());       //将结果存入全局变量
        }
    }
    ...
    public void RunRemoteFun(Node m_XmlNode)
    {
        ...
        try {
            ...
            if(typename.equals("DIRECT_EXECUTE_SUCCESS"))
                parentProcess.ShowError("远程过程调用失败，错误信息：" + e.getMessage());
        }
        if(ret != null)
        {
            Node retnode = FindXmlNode(m_XmlNode.getFirstChild(), "RET");
            ...
            {
                parentProcess.ShowError("远程调用返回异常对象，信息为：" + ((Exception) ret).getMessage());
            }
            else
                parentProcess.pro_config.setVal(GetNodeText(retname), ret.toString());       //将结果存入全局变量
        }
    ...
    //运行一个远程过程
    {
        ...
        IService reservice = (IService) namingContext.lookup(parentProcess.pro_config.sValRemoteUrl);
        Object ret = null;
