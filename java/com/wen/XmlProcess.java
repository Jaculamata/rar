...
import com.wen.remote.JmiItf.IService;
...
class CONNECTINFO              //进程数据库连接信息类
{
   ...
   public Statement State;            //当前连接的陈述
...
class XMLRUNINFO                        //存储当前正在运行的XML文件的一些信息
{
   ...
   public boolean bCancel;  //点击“x”，取消测试
   public boolean bWaitUser;              //等待前台用户选择
...
class Xml_Info
{
   ...
   String xmlStr = "";
   public Xml_Info(String infilename, String inxmlStr)
   {
      this.filename = infilename;
      this.xmlStr = inxmlStr;
   }
}
@WebService
public class XmlProcess {

   public Mygui  frame;                              //主界面
   public Queue  xml_Queue;                         //XML队列
   public String User_ID;                         //测评用户ID
   public CallBack  messRet;                      //消息返回回调类
   public boolean  StopProcess;                   //终止处理
   ...
   public XmlProcess(String UID)
   {
      User_ID = UID;                      //用户信息
      InitProcessInfo();                 //初始化引擎运行信息
   }
   public XmlProcess(String UID, ProClass userconfig)
   {
      User_ID = UID;                      //用户信息
      InitProcessInfo();                 //初始化引擎运行信息
      pro_config = userconfig;
   }
   public XmlProcess(String UID, CallBack CallMess)
   {
      User_ID = UID;
      InitProcessInfo();
      messRet = CallMess;
   ...
   @WebMethod
   public boolean push_xml(String filename, String xmlStr)
   {
      synchronized (xml_Queue)
      {
         xml_Queue.offer(new Xml_Info(filename, xmlStr));
         System.out.println("XmlProccess push Queen "+filename);
      }
      return true;
   }
   @WebMethod
   public boolean push_xml(String xmlStr)
   {
      synchronized (xml_Queue)
      {
         xml_Queue.offer(new Xml_Info("default.xml", xmlStr));
      }
      return true;
   }
   @WebMethod
   public void run_xml()       //运行xml
   {
      Xml_Info currXml = null;
      while(!StopProcess)           //当没有终止运行
      {
         synchronized (xml_Queue) {
            if (xml_Queue.isEmpty()) {
               try {
                  Thread.sleep(100);
                  continue;
               ...
               currXml = (Xml_Info) xml_Queue.poll();
               System.out.println("currXml id"+currXml.filename);
            }
         }
         InitXmlInfo(currXml.filename, currXml.xmlStr);            //初始化当前XML运行信息
         frame.AppendMess("开始测试文件:" + currXml.filename + "...", true);
         boolean xml_return = Run(false);
         if(stXmlRunInfo.bCancel) {
            frame.AppendMess("文件:" + currXml.filename + "被取消执行...",true);
         } else {
            if(xml_return)          //返回值代表失败或者成功
            {
               frame.AppendMess("文件:" + currXml.filename + "执行成功...",true);
            }
            else
            {
               frame.AppendMess("文件:" + currXml.filename + "执行失败...",false);
            }
         }

         ClearXmlInfo();                        //清除当前XML运行信息
         if(StopProcess)
            break;
      }
      frame.AppendMess("程序已终止...", true);
   }
   @WebMethod
   public boolean stop_xml()          //停止执行
   {
      if(stXmlRunInfo != null)
   ...
   @WebMethod
   public boolean restart_xml()           //继续执行
   {
      if(stXmlRunInfo != null)
      {
   ...
   public boolean push_CtrlMsg(CtrlMsg  usermsg)
   {
      synchronized (this)
      {
         frame.AppendMess(usermsg.getAllMsg()+"???", true);
         if(usermsg == null) //用户点击“x”，传空
         {
            stXmlRunInfo.bStop = true;
            stXmlRunInfo.bCancel = true;
            return true;
         }
         this.notifyAll();                         //当用户有反馈后继续执行
         Node m_XmlNode = stXmlRunInfo.xCtrlNode;     //得到当前控制节点
         if(m_XmlNode == null)
            return false;
         Node retNode = FindXmlNode(m_XmlNode, "RET");   //寻找RET节点
         if(retNode == null)
            return true;
         Node tmp = retNode.getFirstChild();
         int index = 0;
         while(tmp != null)
         {
            if(tmp.getNodeName().equals("VALNAME"))
            {
               pro_config.setVal(GetNodeText(tmp).trim(), (String)usermsg.getAllMsg().get(index));
               ++index;
            }
   ...
   public boolean import_CtrlMsg(CtrlMsg  usermsg)
   {
      synchronized (this)
      {
         try
         {
            if(messRet != null)
            {
               messRet.callbackCtrl(User_ID, usermsg);   //插入控制消息
               this.wait(5 * 60 * 1000);             //等待超时时间半分钟
            }
         } catch (InterruptedException e) {
            e.printStackTrace();
         }
   ...
   {
      //执行当前文件时  将脚本的id传入  并赋值
//      stXmlRunInfo.sXmlFileName=currName;
      if(!bTransfer)
         ShowSuccess("开始执行...");
      ...
      if(stXmlRunInfo.bCancel)
      {
         frame.AppendMess("测试取消...", true);
         if(messRet != null)
            messRet.callback(User_ID, stXmlRunInfo.sXmlFileName, "测试取消...");
      } else if (cSqlCase.stCaseResult.bSuccess) {
         //System.out.println("测试通过");
         frame.AppendMess("测试通过...", true);
         if(messRet != null)
            messRet.callback(User_ID,stXmlRunInfo.sXmlFileName, "测试通过...");
      } else {
         //System.out.println("测试失败");
         frame.AppendMess("测试失败...",true);
         if(messRet != null)

            messRet.callback(User_ID,stXmlRunInfo.sXmlFileName, "测试失败...");
         System.out.println("XMLPROCESS "+messRet.toString()+"user "+User_ID+" id "+stXmlRunInfo.sXmlFileName);
      }
      return cSqlCase.stCaseResult.bSuccess;
   }
   private  void dynamicLoadJdbc(String path) throws Exception {
      File file = new File(path);            //JDBC的路径
      URL url = file.toURI().toURL();
      String classname = pro_config.sValDriveName;
      URLClassLoader ucl = new URLClassLoader(new URL[] { url });
      Driver d = (Driver)Class.forName(classname, true, ucl).newInstance();
      DriverShim driver = new DriverShim(d);
      DriverManager.registerDriver(driver);
   }
   public void InitProcessInfo()           //初始化类运行时信息
   {
      frame = new Mygui(this);                    //主界面初始化
      frame.setVisible(true);                    //主界面可视化
      xml_Queue = new LinkedList<Xml_Info>();      //XML队列
      pro_config = new ProClass();                //前台配置类
      StopProcess = false;                          //终止处理
      messRet = null;
      ...
      try
      {
         ...
         pro.load(in);
         in.close();
         jdbc_path = pro.getProperty("PATH");
      }
      catch (Exception e)
      {
         e.printStackTrace();
      ...
      try
      {
         dynamicLoadJdbc(jdbc_path);         //动态加载JDBC驱动
      }
      catch (Exception e)
      {
         e.printStackTrace();
   ...
   public void InitXmlInfo(String filename, String xmlStr)              //单个XML运行之前的初始化
   {
      cLoop = null;                                     //XML文件的循环结构
      stConnectArry = new ArrayList();                //主线程已经建立的连接
      stThreadArry = new ArrayList();                //主线程已经开启的所有子线程
      stXmlRunInfo = new XMLRUNINFO();              //当前XML文件运行信息
   ...
   public void ClearXmlInfo()           //一个XML运行结束之后的信息清除，以便下一个XML的运行
   {

      //待添加
      pro_config.valclass.clear();       //清空全局变量
   ...
   public boolean AnalyseNode(Node m_XmlNode)
   {
      ...
      switch(m_XmlNode.getNodeName().toUpperCase())             //将m_XmlNode节点的标签统一改成大写
      {
         ...
         case "TEMPVAL":               //取消该关键字
            sTempStr = "";
            if(stXmlRunInfo.cRunInfo.containsKey("TEMPVAL"))
               sTempStr = stXmlRunInfo.cRunInfo.get("TEMPVAL").toString();
            if(stXmlRunInfo.cRunInfo.containsKey("TEMPVAL"))
               stXmlRunInfo.cRunInfo.remove("TEMPVAL");       //防止自身被替代
            ...
            if (sValues.equals(""))
            {
               stXmlRunInfo.cRunInfo.put("TEMPVAL","");
            }
            else if (sValues.equals("@"))
            {
               sValues = sTempStr;
               sValues = ReplaceRunInfo(sValues);
               stXmlRunInfo.cRunInfo.put("TEMPVAL",sValues);
            }
            else
            {
               sValues = ReplaceRunInfo(sValues);
         ...
         case "RUNIMPORT":
            ...
            break;
         case "RUNREMOTE":
            RunRemoteFun(m_XmlNode);
            break;
         case "CTRLMSG":
            StartCtrlMsg(m_XmlNode);
            break;
         default:
   ...
   public boolean  ExecuteSQL(String m_Sql)
   {
      ...
      try
      {
         ...
         if (!m_noShow)
         {
            frame.AppendMess(m_Sql, true);
            if(messRet != null)
               messRet.callback(User_ID,stXmlRunInfo.sXmlFileName, m_Sql);
         }
         stConnectINfo.isOpenResult = false;
         time1 = new Date();                         //为执行语句计时
         stConnectINfo.State = stConnectINfo.cn.createStatement();
         if(stConnectINfo.State.execute(m_Sql))                              //有结果集返回
         {
   ...
   public boolean CheckResultRowEx(Node m_XmlNode, int rownums)
   {
      try
      {
         ...
         while(m_row <= rownums)				//开始比较结果集
         {
            ...
            {
               if (!CheckColumnEx(m_XmlNode.getFirstChild(), m_row, m_colNums))   //带有COLUMNNUMS的检查
               {
                  break;
               }
            }
   ...
   public boolean CheckTypeValue(String sValue)
   {
      ...
      stXmlRunInfo.cCurrentSqlCase.stCaseResult.bExpResult = true;
      ShowError("脚本中存在非法的预期执行结果关键字(就是TYPE关键字包含部分的字符串不是已被定义的)");
      return false;
   }
   /// <summary>
   /// //用来得执行LOOP循环
   ...
   private void StartLoopNode(Node m_XmlNode)
   {
      ...
      try
      {
         ...
         m_xmlTempNode = this.FindXmlNode(m_XmlNode.getFirstChild(), "FORTIMES");
         if (m_xmlTempNode == null)
         {
   ...
   public String ReplaceRunInfo(String sql)
   {
      ...
      if(stXmlRunInfo.cRunInfo.containsKey("TEMPVAL"))
      {
         tmp = stXmlRunInfo.cRunInfo.get("TEMPVAL");
         stXmlRunInfo.cRunInfo.remove("TEMPVAL");
      ...
      if(cLoop != null)
      {
         ...
         while (index != -1)
         {
            ...
            int iTemp1 = sTemp.indexOf("FORTIMES");
            int iTemp2 = GetUpTimes(iTemp1-1);
            if (iTemp2 != -1)
            {
               sTemp = sTemp.substring(0, iTemp1 + "FORTIMES".length());
      ...
      if(tmp != null)
      {
         stXmlRunInfo.cRunInfo.put("TEMPVAL",tmp.toString());
      }
      return sql;
   }
   ...
   public void ShowError(String mess)
   {
      ...
      stXmlRunInfo.cRunInfo.put("CASERESULT", "FALSE");
      if (stXmlRunInfo.cCurrentSqlCase.stCaseResult.bExpResult)
      {
         cSqlCase.stCaseResult.bSuccess = false;

         if (!pro_config.bValIsErrRun && !stXmlRunInfo.bClearEn)//如果不允许继续运行下面一个结点的值
            stXmlRunInfo.bStop = true;
         frame.AppendMess("失败消息:" + mess, false);
         if(messRet != null)
            messRet.callback(User_ID,stXmlRunInfo.sXmlFileName, "失败:" + mess + "\n");
      }
      else
      {
         frame.AppendMess("成功消息:" +  mess,true);
         if(messRet != null)
            messRet.callback(User_ID,stXmlRunInfo.sXmlFileName, "成功消息:" + mess + "\n");
      }
   }
   public void ShowSuccess(String mess)
   {
      ...
      if(messRet != null)
      {
         messRet.callback(User_ID,stXmlRunInfo.sXmlFileName,stXmlRunInfo.sXmlFileName + "成功消息:" + mess + "\n");
      }
   ...
   private void RunServerCmd(Node m_XmlNode)
   {
      ...
      SerCmdEx cmdex = new SerCmdEx(pro_config.sValOSUid, pro_config.sValOSPwd,pro_config.sValServer,22);
   ...
   private void RunServerCmdEx(Node m_XmlNode)                      //该函数获取执行结果到脚本变量中，交互由用户控制
   {
      String cmd = "";
      Object[]  e = new Exception[1];
      if(pro_config.sValOS.equals("LINUX"))
      ...
      String retstr = cmdex.execute_str(cmd, e);
      if(retstr != null)
      {
         Node retnode = FindXmlNode(m_XmlNode.getFirstChild(), "RET");
         if(retnode == null)
            return ;
         Node retname = FindXmlNode(retnode.getFirstChild(), "VALNAME");
         pro_config.setVal(GetNodeText(retname), retstr);                   //将结果存入全局变量
      }
   }
   ...
   {
      Node jar_temp = FindXmlNode(m_XmlNode.getFirstChild(), "JARNAME");
      String jarname = GetNodeText(jar_temp);       //获取JAR的路径和名称
      Node class_temp = FindXmlNode(m_XmlNode.getFirstChild(), "CLASSNAME");
      String classname = GetNodeText(class_temp);       //获取要加载的类名称
      Node fun_temp = FindXmlNode(m_XmlNode.getFirstChild(), "FUNNAME");
      String funname = GetNodeText(fun_temp);       //获取要加载的函数名称
      String[] inargs = getAllargs(m_XmlNode);      //获取所有参数
      Object ret = null;
      try {
         ret = MyUtil.RunJarFun(jarname, classname, funname, inargs);
      } catch (Exception e) {
         ShowError("第三方Jar调用失败，错误信息：" + e.getMessage());
      }
      if(ret != null)
      {
         Node retnode = FindXmlNode(m_XmlNode.getFirstChild(), "RET");
         if(retnode == null)
            return ;
         Node retname = FindXmlNode(retnode.getFirstChild(), "VALNAME");
         pro_config.setVal(GetNodeText(retname), ret.toString());       //将结果存入全局变量
      }
   }
   private String[] getAllargs(Node m_XmlNode)
   {
      ArrayList List = new ArrayList();
      Node tmp = m_XmlNode.getFirstChild();
      while(tmp != null)
      {
         if(tmp.getNodeName().equals("ARG"))
         {
            List.add(ReplaceRunInfo(GetNodeText(tmp).trim()));
         }
         tmp = tmp.getNextSibling();
      }
   ...
   private String[] getAllchoose(Node m_XmlNode)
   {
      ArrayList List = new ArrayList();
      Node tmp = m_XmlNode.getFirstChild();
      while(tmp != null)
      {
         if(tmp.getNodeName().equals("BUTTON"))
         {
            List.add(ReplaceRunInfo(GetNodeText(tmp).trim()));
         }
         tmp = tmp.getNextSibling();
   ...
   public void RunRemoteFun(Node m_XmlNode)
   {
      Node jar_temp = FindXmlNode(m_XmlNode.getFirstChild(), "FUNNAME");
      String funname = GetNodeText(jar_temp);       //获取远程过程名称
      Node type_temp = FindXmlNode(m_XmlNode.getFirstChild(), "TYPE");
      String typename = GetNodeText(type_temp);       //获取远程过程期待结果类型
      String[] inargs = getAllargs(m_XmlNode);      //获取所有参数
      Object ret = null;
      try {
         ret = RunRemoteFunhelp(funname, inargs);
      } catch (Exception e) {
         if(typename.equals("DIRECT_EXECUTE_SUCCESS"))
            ShowError("远程过程调用失败，错误信息：" + e.getMessage());
      }
      if(ret != null)
      {
         Node retnode = FindXmlNode(m_XmlNode.getFirstChild(), "RET");
         if(retnode == null)
            return ;
         Node retname = FindXmlNode(retnode.getFirstChild(), "VALNAME");
         if(ret instanceof Exception )    //返回异常
         {
            if(typename.equals("DIRECT_EXECUTE_SUCCESS"))
               ShowError("远程调用返回异常对象，信息为：" + ((Exception) ret).getMessage());
            else
               pro_config.setVal(GetNodeText(retname), "false");
         }
         else
            pro_config.setVal(GetNodeText(retname), ret.toString());       //将结果存入全局变量
      }
   }
   public Object RunRemoteFunhelp(String funname, String[] inargs) throws NamingException, RemoteException
   //运行一个远程过程
   {
      Context namingContext = new InitialContext();
      // 检索指定的对象。 即找到服务器端相对应的服务对象存根
      IService reservice = (IService) namingContext.lookup(pro_config.sValRemoteUrl);
      Object ret = null;
      ret = reservice.Remoteexe(funname, inargs );
      return ret;
   }
   public void StartCtrlMsg(Node m_XmlNode)
   {
      Node type_temp = FindXmlNode(m_XmlNode.getFirstChild(), "CTRLTYPE");    //交互类型字段
      String typename = GetNodeText(type_temp);          //获取字段名称
      String[] inargs = getAllargs(m_XmlNode);          //获取所有参数
      String[] inchoose = getAllchoose(m_XmlNode);       //获取所有要在前台显示的按钮文本
      //构造交互消息
      CtrlMsg inmsg = new CtrlMsg();
      inmsg.setType(typename);
      inmsg.setAllMsg((List) Arrays.asList(inargs));
      inmsg.setAllch((List) Arrays.asList(inchoose));
      stXmlRunInfo.xCtrlNode = m_XmlNode;
      //传送交互消息
      import_CtrlMsg(inmsg);
   }
}